...
class TestMovieVoteAverage():
    ...
    def test_getmoviedetails(self):
        movie_average_details_response = requests.get(self.url)

        status_code = movie_average_details_response.status_code
        assert status_code == 200

        movie_average_json = movie_average_details_response.json()
